...
exports.getUser = (req, res) => {
    ...
    .then(doc => {
        ...
        return db.collection('players').where('userID', '==', req.params.userID).get();
    })
    .then(data => {
...
exports.addPlayer = (req, res) => {
    ...
    .then(doc => {
        if (!doc.exists) res.status(404).json({ error: 'User not found'});
        return db.collection('players').where('userID', '==', req.params.userID).where('bracket', '==', req.body.bracket);
    })
    .then(doc => {
        if (doc.exists) res.status(400).json({ error: 'Player already exists'});
